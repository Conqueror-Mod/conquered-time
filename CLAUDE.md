...
#### Planned — approved design, not yet built (2026-07-04)
Full spec in `docs/PLAN-pomodoro-tooltips-onboarding.md` — decisions there were confirmed with the user via Q&A; build order A→B→C:
- **A. Tooltip system** — shared hover/focus tooltip component (`data-tip` attribute, single delegated listener, themed, reduced-motion aware); v1 covers Tracker fields (Task Name, Session Label, Description incl. Dispatch writeback), Profile fields (Work State, Break Style, Email), Dispatch Log-a-Task, all Dashboard buttons.
- **B. Pomodoro break style** — per-profile `break_style` (`state` default | `pomodoro`) + preset (`classic` 25/5, `extended` 50/10, `gentle` 90/15; presets defined in `policies.ts`), selector on the **Profile page** next to Work State. **Warnings-only:** Pomodoro replaces the live tracker break-cadence warnings and adds a Dispatch cycle timer + sidebar countdown chip; the **audit engine stays on state policy** (do not touch `countAuditDiscrepancies` or the seed's expected count). Break alerts **prompt to punch** (themed toast with Start Break when visible; Windows notification when tray'd) — never auto-punch. Lunch warnings stay state-policy-driven.
- **C. First-time onboarding wizard** — guided coach-marks (dimmed overlay + spotlight + anchored cards) across dashboard→profile→companies→tracker→dispatch→finish; steps keyed by page, resumable across `loadFile()` navs via sessionStorage. Per-profile one-shot `ui_onboardingDone` flag (seed sets it), re-runnable from Settings → About; modal order on first login: encryption notice → wizard → audit login-notice. Profile step includes Work State + break-style choice; final step points out hover tooltips.
