...
- **BUG — time format doesn't re-render live.** Changing the 12h/24h display preference in Settings only affects *future* time punches; already-rendered timestamps in the tracker/global-log keep their old format until the page reloads. The stored format is always 24h (gotcha #8) — this is purely a display refresh: on time-format change, re-render existing rows instead of only applying to new ones.
- **Time Tracker DBA-deletion selector.** Add a per-company selector to clear/delete the time tracker for a chosen company. Today's Settings → Data "Time Clock Clear" is global (all companies); this wants per-company scoping (pick the company, clear only its entries).
- **Dispatch — auto-fill active Task Name.** On starting the Dispatch timer, pre-fill the active session's current Task Name into the timer field, while still keeping the dropdown for when multiple tasks were queued during the session. Currently you must re-select the task from the dropdown every time you restart the timer for the same task — cumbersome for repeated identical tasks.
- **Tracker table — manual cell/column resizing.** Let the user resize individual cells/columns in the time-punch table; the table keeps its overall size, only the internal column widths change.
- **First-login encryption acknowledgement.** On first login, show a one-time acknowledgement informing the user their profile data is encrypted at all times, with a "Never Show Again" option (persist the dismissal so it doesn't reappear).
- **Richer Description field in the tracker.** The time-tracker row Description field is very flat but can occasionally hold a lot of text. Make it more dynamic/expandable (similar to the Companies page detail fields) for in-app editing, while still rendering **flattened** in PDF exports and emailed reports (the export/report layout must not break on long or multi-line descriptions).

#### Feature Add
- **Tray + launch-at-startup** ✅ (PR #21 + PR #23 + PR #27) — system `Tray` (Open / Lock Session / Backup Now / Quit; click+dblclick restore the window), opt-in **Close to Tray** toggle (close hides the window and keeps `sessionKey`/`sessionUser` alive so the scheduled-report poller keeps firing; an `isQuitting` flag set by tray Quit / menu Quit / `before-quit` bypasses the hide; default behavior unchanged — close still quits), and **Launch at Startup** toggle (`app.setLoginItemSettings({ openAtLogin, path: process.execPath })`; `win:set-launch-at-startup` IPC). **Both settings are app-global, not per-profile** (PR #23): launch-at-startup's source of truth is the **OS login item itself** (`win:get-launch-at-startup` reads `app.getLoginItemSettings().openAtLogin` — no app storage); close-to-tray lives in **`<ROOT_DATA_DIR>/app-prefs.json`** via `getAppPref/setAppPref` (`win:get/set-close-to-tray`), read by `main.js` at window-close with no vault loaded. This is why the toggles appear and work identically in **both** the in-app Window settings tab AND the **pre-auth login settings tab** (the login screen has no vault, so per-profile vault keys would have been UI-only there). Do **not** move these back into the per-profile `app_settings` vault. **Start minimized to tray** ✅ (PR #27): when the OS auto-launches the app (login item registers a `--startup` marker arg → `STARTED_AT_LOGIN`) AND the `startMinimized` app-pref is on, `whenReady` skips the splash and leaves the window hidden in the tray; gated on `STARTED_AT_LOGIN` so manual opens are unaffected. Toggle in both Window settings surfaces (`win:get/set-start-minimized`). Still TODO: re-verify launch-at-startup + start-minimized with a real Windows login after the first packaged NSIS build (a dev run registers `electron.exe` as the login item — packaged builds resolve `process.execPath` to the real installed `.exe` automatically).
