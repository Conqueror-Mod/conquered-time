...
│   │   │   ├── dashboard.html        # Stat chips, mini galaxy web, recent activity, quick actions
│   │   │   ├── companies.html        # Company Galaxy (packed-bubble web), company CRUD modal
...
- `entries:list/save/delete/all/summary`
  - **`entries:delete` (session delete)** deletes one time entry + its entry-scoped `task_items`, snapshotting the vault BEFORE the delete via `performSafetySnapshot('session-delete')` (data-safety-net protected class, mirrors `companies:delete`). Wired to the Global Log row right-click menu.
...
- **CSP** (identical `<meta>` on all 11 pages): `default-src 'self'`, `script-src 'self'` (**no `'unsafe-inline'`** — hardened in v3.6), `style-src 'self' 'unsafe-inline'`, `font-src 'self' data:`, `img-src 'self' data:`, `object-src 'none'`, `base-uri 'self'`. Fonts are bundled locally (no `fonts.googleapis.com`/`fonts.gstatic.com` origins) — `font-src 'self'` covers the local `.woff2` files and `data:` covers the base64-inlined Inter used in the PDF-export window (which inherits the opener's CSP). `'unsafe-inline'` is **still retained for `style-src` only** — inline `style=""` attributes are pervasive, can't be nonced, and style injection is a far weaker vector; removing it was judged not worth the churn.
  ...
  - **Shared context menu — `Shell.contextMenu(ev, items)`** (`installContextMenu()` in `shell.js`, injected on every inner page). One reusable `#ct-context-menu` element; `items` is a declarative list `[{ label, action, danger?, disabled?, hidden? } | { separator:true } | null]` (hidden dropped, separators collapsed), CSP-safe delegated dispatch, cursor-positioned + viewport-clamped, dismissed on outside-click/scroll/Escape/blur. Right-click menus are wired on: **Global Log** rows (Open in Tracker / Export PDF / Copy summary / Delete session), the **Companies list panel** (Open Tracker / Edit / Edit Color / Delete — reuses the galaxy `ctx*` handlers; the `#ctx-color-pick` was switched to `position:fixed` + `ctxAnchorX/Y` so Edit Color anchors correctly from either the galaxy bubble or the list), the **invoice ledger** (Save PDF / Email / Mark Paid|Unpaid / Void — shares `runLedgerAction()` with the row buttons; status-gated via `hidden`), and **tracker rows** (Edit / Duplicate / Clear row, filled rows only). The Galaxy's own two bespoke menus (`#ctx-menu`/`#galaxy-ctx-menu` in companies.html) predate this and still work; folding them onto `Shell.contextMenu` is an optional follow-up. Use `Shell.contextMenu` for any NEW right-click menu.
...
### 4. Canvas visualizations need `ResizeObserver`, not fixed dimensions
Both the dashboard mini web and the full Companies page web previously failed to recenter/resize on window maximize/restore. Fixed by wrapping canvas init in a `ResizeObserver` on the parent container and recomputing the layout from live `wrap.clientWidth/clientHeight` — never cache the canvas center at init time. (The v3.20 Galaxy engine in `src/renderer/bubble-web.ts` keeps this rule: its controller re-runs `layout()` from a ResizeObserver.)
...
**Scope & recipients (v3.15):** scheduled reports have a company scope (`email_schedule_scope` in `app_settings`): `''` = all companies (one combined report), `'each'` = one report per company that has work in the period, `'<rowid>'` = a single company. Companies carry an optional `report_email` in their encrypted blob ("Report Recipient" in the company modal) — per-company sends go there, falling back to the SMTP default recipient. Empty-period behavior (v3.14.1): any scope with zero entries skips the send but still advances `email_schedule_last_sent` (otherwise the catch-up loop retries every 5 minutes and the next report stretches over the idle period); in `'each'` mode idle companies are always skipped even on force/Send Now. Partial failure in `'each'` mode: if at least one company sent (or all were skipped), `last_sent` advances and failures land in `email_schedule_last_error`; if nothing sent and a real send failed, it throws so the whole window is retried.
...
- **Data safety net (v3.21, `src/main/backups.ts`)** — two-part protection against losing data to a misclick. (1) **Pre-action safety snapshots:** every destructive handler now snapshots the vault *before* the mutation, via `performSafetySnapshot(reason)`, which writes `safety-<stamp>__<slug>.db` in the profile's `backups/` dir. These are a **separate protected class** from the routine `vault-<stamp>.db` autosave ring — the two prune passes only ever touch their own prefix (autosave cap 30, safety cap 15), so 30 autosaves after a clear can't evaporate the snapshot taken before it. Wired into `db:clear-timeclock`, `db:clear-timeclock-company`, `db:clear-companies` (in `ipc/settings.ts`), `companies:delete` (in `ipc/companies.ts`), and `backup:restore` (reason `before-restore`) — each does `persistDB(); performSafetySnapshot(reason)` first so the snapshot captures the exact pre-action state (verified: a 13-company snapshot vs 12 live after a delete). **Gotcha this fixes:** the old code called `performBackup()` *after* the DELETE, backing up the damage; the post-action `performBackup()` stays as the routine autosave. (2) **Portable vault export** (`backup:export-portable`): a save-dialog that copies the **encrypted** `vault.db` to a user-chosen location — still needs the account password to open, so it's safe anywhere; no plaintext/decrypted bundle by design. UI in Settings → Data: an **Export Vault** card, safety snapshots badged (🛡 + humanized reason) in the Backup Library, and a **full-clear export nudge** (`showFullClearNudge()` in shell.js — Cancel / Continue Anyway / Export & Continue) because `db:clear-full` deletes the whole profile dir including `backups/`, so an in-profile snapshot can't help there — the export is its safety net (choosing Export & Continue aborts the wipe if the save is cancelled). `backup:list`/`preview`/`restore` accept both filename classes via a shared `isBackupName` guard (`VAULT_RE`/`SAFETY_RE`, path-traversal-safe).
- **Tray punch + global hotkey (v3.21, `src/main/punch.ts`)** — Clock In / Clock Out from the tray menu or a system-wide hotkey (default `Ctrl+Alt+P`, toggle semantics), without opening the app — works while hidden in the tray. **Clock-in repeats the last task**: most recent company + its last row's Task Label/Name appended onto today's entry for that company (satisfying the "label+name required" rule; no history → opens the app instead of writing a blank punch). Clock-out closes the open row, rolls `total_mins`, and stops running task_items (C6 rule). **Locked/no session → open the app + prompt sign-in; never queue a plaintext punch outside the vault.** Key internals: `findOpenPunch()` asks the stricter question "is a punch OPEN?" — `findActiveEntry()`'s fast path returns the `activeEntryId` entry even when every row is closed (the entries:get-active semantics the tracker relies on; both helpers live in punch.ts and entries:get-active now delegates to it). After any main-process punch the renderer gets `punch:changed`; the tracker reloads the affected session in place (its stale `updated_at` token would otherwise reject its next autosave). The tray menu is rebuilt on punch changes + a 20s interval (Windows has no pre-popup menu hook). The hotkey is an **app-global pref** (`punchHotkey` in app-prefs.json — an OS registration, not profile data), configurable via a key-capture input in Settings → Window (a modifier is required; a colliding/invalid binding is rolled back, never committed); IPC `win:get/set-punch-hotkey`. `tsconfig.main.json` also copies `time-parse.js` into dist-main for main-side `computeDiffMins` (midnight wrap).
- **Auto-updater (v3.17, `src/main/updater.ts`)** — in-app updates via `electron-updater` against the GitHub Releases feed (`build.publish` in package.json → `Conqueror-Mod/conquered-time`). Opt-in flow: **check → download (with progress) → restart & install**; `autoDownload=false`, `autoInstallOnAppQuit=true`. A silent check fires ~10s after launch (packaged only). State is pushed to the renderer over the `update:status` event and reflected in **Settings → About → Updates** (progress bar + action button). IPC: `update:check/download/install/status`. **Guarded on `app.isPackaged`** — in dev every handler no-ops with a `'dev'` status (electron-updater needs the bundled `app-update.yml`, absent in dev). Publish with `npm run release` (`electron-builder --publish always`, needs `GH_TOKEN`) → uploads installer + `latest.yml`. Unsigned during beta (SmartScreen warns on first install). Replaced the old `app:check-update` browser-download poll (the IPC handler is removed from the new build). **`version.json` is intentionally KEPT** at repo root as a legacy bridge: pre-3.17 installs still have the old `app:check-update` baked into their binary and poll `raw.githubusercontent.com/.../master/version.json` — deleting it 404s their check ("Invalid response from update server"). It's pinned at `3.17.0` (the version that introduces the auto-updater) so stragglers get nudged onto the self-updating track; no need to bump it every release.
...
- **Company Galaxy (v3.20, `src/renderer/bubble-web.ts` — window.BubbleWeb)** — the company web on Dashboard (mini) and Companies (full). Packed bubbles (static greedy-spiral layout — no physics loop), **galaxies** = rows grouped by `hier_company` (hours rolled up over a 30d/90d/All preset, `ui_webRange`, shared by both pages). Multi-project galaxy click → in-place expand into its **systems** (pin+relax, never repack — position stability); system click → **zoom** to level 1 (breadcrumb `#web-crumb` backs out; ~250ms tween gated on reduced-motion); system click at L1 → **planets** (Platform · NavID · login). Single-project galaxies click straight to the tracker; one-galaxy vaults skip level 0. Ended/window-idle galaxies fold into a grey **Archive** satellite. **Identity colors**: stable hue per galaxy (min-rowid hash → curated palette; deutan/protan-safe palette when the colorblind a11y mode is on), saturation fades with recency; right-click menus (galaxy + system variants) include Edit Color — a galaxy color writes the same `color` into every row's encrypted blob, and `saveCompany()` deliberately carries `color` through form saves so edits can't wipe it. Search dimming propagates down. The company list panel mirrors the grouping as a two-level accordion. Dashboard multi-project click → Companies page pre-zoomed (sessionStorage `web_zoom_galaxy`). HARNESSED sizing at every level: ≤~40% area budget, per-bubble floor + ceiling (whale-capped); sizes are ordinal-honest, tooltips (bubble-anchored, never cursor-following) carry exact numbers. Plan/history: `docs/PLAN-company-galaxy.md`, mocks `docs/mock-bubble-web.html` + `docs/mock-galaxy-web.html` (PRs #133–#137). **Fly-through polish (PR #148):** drill-in `withZoom` now pushes the camera INTO the clicked bubble (transform-origin anchored, snappy ~300ms); single-project galaxies `warpOut` (dive ~4× + fade) before navigating to the tracker; `onSelect`/`onHover` callbacks let the Companies page glow (`.glow-focus` pulse / `.hover-glow` steady) the list row matching a clicked/hovered system; archive kids sized via `harness()` (floor 15). JS motion gates on `data-reduced-motion`; CSS motion is killed globally in `base.css` — do NOT add per-page reduced-motion CSS.
- **Company colors everywhere (`BubbleWeb.colorMap`)** — the Galaxy identity hue as a small `.co-dot` (in `components.css`) before company names on **Recent Activity** (dashboard), **Global Log**, and the **invoice ledger**; Insights' client-mix donut already used identity color (v3.22). The single primitive is `BubbleWeb.colorMap(companies, entries) → { colorFor(companyId) }` in `bubble-web.ts` (groups by `groupKey`, recency from latest `log_date`, defers to `identityCss`; neutral fallback for unknown/deleted). dashboard.ts + insights.ts were refactored off their duplicated inline copies onto it — **keep all identity color going through `colorMap`/`identityCss` so surfaces can't drift.** `global-log.html`/`invoices.html` load `bubble-web.js` (+`canvas-text.js`) just for the helper. `invoices:list` now returns `company_id` (pulled from the decrypted invoice `doc`) so the ledger can color reliably.
...
- Dev seed script (`npm run seed`, **v4.0 — stress fixture**) — wipes `dev-data/`, creates `devuser`/`devpass123` with `dev_mode=1` (TOTP bypassed, `work_state=TX` → default policy), then seeds a designed measurement instrument: **10 companies** (2 canonical clean baselines Zenith/Apex; unicode/emoji `Café Müller 東京 🚀`; an **XSS canary** `<script>…</script>` company whose `onerror` probes set `document.title='XSS-FIRED'` if any sink is unescaped; a 120-char-name overflow probe; a name-only minimal co; whitespace-padded name; quotes `O'Brien & Sons "Quality"`; high-volume `Meridian Ops`; a **3-row Vertex Group galaxy trio** (shared `hier_company` — the Company Galaxy multi-project probe, P13); and **Pristine Control Co inserted LAST** — zero entries, the run-app `verify-cursed-path` target — **13 companies total**) and **106 entries** (6 canonical incl. the discrepancy session with EXACTLY 6 audit issues; an edge-probe session: desc-only row, midnight-crossing 23:30→00:15, 00:00→23:59 stored-vs-span drift, custom non-list Task Label, long multiline desc; a **legacy plaintext entry** exercising the at-login encryption migration; a future-dated entry; a same-date pair; 92 clean volume entries; 3 clean galaxy entries with staggered recency). **12 task_items** incl. 2 orphans (dead `entry_id`), a stale in-progress break, 0s and 25h tasks, an emoji/HTML label. 3 backup fixtures (`vault-<stamp>.db`). Settings under real `ui_*`/`win_*` keys + `ui_encryptionNoticeAck=1` (so modals don't block automated sweeps). Prints a 17-assert **self-check ledger** + a **verification packet** with a STRESS LEDGER (probes P1–P13). The discrepancy session carries EXACTLY 6 issues, but the fixture-wide audit total the seed's self-check asserts (and `audit:count` returns on a fresh seed) is **7** — the desc-only edge-probe row also flags `no_clock_in` since the C3 fix made the skip-filter desc-aware. `computeExpectedDiscrepancies()` (in `test/vault-fixture.js`, imported by both the seed's self-check and the differential oracle `test/audit-oracle.props.test.js`) mirrors `countAuditDiscrepancies()` in `src/main/audit.ts` — keep in sync; the oracle mechanically enforces agreement on randomized vaults. The seed builds its vault through the same `test/vault-fixture.js` builders the property tests use. Never ships in production. **Note:** the audit engine detects only the 6 types — it does NOT flag row overlaps or stored-vs-span mismatches.
...
#### Tooltips / Pomodoro / Onboarding cluster ✅ (v3.13.0 — PRs #88, #92 (reopened #89), #90, #91)
Built per `docs/PLAN-pomodoro-tooltips-onboarding.md` (decisions confirmed with the user; keep the constraints below when touching these systems):
- **Tooltip system** — `installTooltips()` in `shell.js`: one shared `#ct-tooltip` element, document-level delegated `mouseover`/`focusin` listeners (CSP-safe, covers injected markup), 400 ms hover delay / instant on keyboard focus, viewport flip + clamp, `textContent`-only. Add tips via `data-tip` (+ optional `data-tip-pos`). v1 copy on Tracker fields, Profile fields, Dispatch Log-a-Task, all Dashboard buttons.
- **Pomodoro break style** — per-profile `break_style` (`state` default | `pomodoro`) + `pomodoro_preset` (`classic` 25/5, `extended` 50/10, `gentle` 90/15; presets in `policies.ts`), stored in the encrypted profile blob, selector on the Profile page next to Work State; `audit:get-policy` exposes `breakStyle`/`pomodoro`. Engine in `components/pomodoro.js` (injected by `Shell.init()`; state in sessionStorage `ct_pomo`; runs only against an open punch): Dispatch cycle panel + sidebar countdown chip; tracker break-cadence warnings follow the preset. **Warnings-only — the audit engine and lunch warnings stay on state policy** (do not touch `countAuditDiscrepancies` or the seed's expected count for this). Break alerts **prompt to punch** (sticky action toast; `app:notify` Windows notification when the window is hidden, click restores) — **never auto-punch**.
- **First-login onboarding tour** — `components/onboarding.js`: 7 coach-mark steps keyed by page (dashboard→profile→companies→tracker→dispatch→global-log→finish), spotlight cutout + anchored card, resumable across `loadFile()` navs via sessionStorage `ct_tour`. One-shot per profile via `ui_onboardingDone` (Finish AND Skip set it; seed pre-sets it); modal order on first login: encryption notice → tour → audit login-notice, chained via `showEncryptionNotice(onDismiss)` — never stacked. Replay from Settings → About ("Setup Guide" section; hidden on the pre-auth login About).

#### Feature Add
- **Tray + launch-at-startup** ✅ (PR #21 + PR #23 + PR #27) — system `Tray` (Open / Lock Session / Backup Now / Quit; click+dblclick restore the window), opt-in **Close to Tray** toggle (close hides the window and keeps `sessionKey`/`sessionUser` alive so the scheduled-report poller keeps firing; an `isQuitting` flag set by tray Quit / menu Quit / `before-quit` bypasses the hide; default behavior unchanged — close still quits), and **Launch at Startup** toggle (`app.setLoginItemSettings({ openAtLogin, path: process.execPath })`; `win:set-launch-at-startup` IPC). **Both settings are app-global, not per-profile** (PR #23): launch-at-startup's source of truth is the **OS login item itself** (`win:get-launch-at-startup` reads `app.getLoginItemSettings().openAtLogin` — no app storage); close-to-tray lives in **`<ROOT_DATA_DIR>/app-prefs.json`** via `getAppPref/setAppPref` (`win:get/set-close-to-tray`), read by `main.js` at window-close with no vault loaded. This is why the toggles appear and work identically in **both** the in-app Window settings tab AND the **pre-auth login settings tab** (the login screen has no vault, so per-profile vault keys would have been UI-only there). Do **not** move these back into the per-profile `app_settings` vault. **Start minimized to tray** ✅ (PR #27): when the OS auto-launches the app (login item registers a `--startup` marker arg → `STARTED_AT_LOGIN`) AND the `startMinimized` app-pref is on, `whenReady` skips the splash and leaves the window hidden in the tray; gated on `STARTED_AT_LOGIN` so manual opens are unaffected. Toggle in both Window settings surfaces (`win:get/set-start-minimized`). ✅ **Verified 2026-06-30** against the first packaged build (v3.8.0): installed from the NSIS installer, toggled launch-at-startup, and confirmed the app auto-launches on a real Windows login (packaged builds resolve `process.execPath` to the real installed `.exe`, not the dev `electron.exe`). **Login-item arg quoting (v3.17):** `loginItemOpts` must **quote** `app.getAppPath()` in its `args` — `setLoginItemSettings` joins args with spaces without quoting, so the repo's own space-containing path (`…\My Projects\…`) got split and `electron.exe` launched with no app → the **default Electron "welcome" window** appeared on every Windows login / RDP reconnect. Dev branch only (packaged passes no app-path arg). Do not drop the quotes.
...
**Elevation system:** 3-layer shadow system (`--shadow-1/2/3` + `--shadow-inset`) gives panels/cards/modals a sense of physical depth without literal 3D. Galaxy-web bubbles use radial-gradient "sphere" fills (light source top-left) rather than flat circles — and are **theme-aware**: `themeColors()` in `src/renderer/bubble-web.ts` switches fill recipes by light (Memoria/Rabanastre) vs dark (Zanarkand/Treno/Nibelheim) themes — light themes get solid saturated fills with white labels (translucent glows vanish on light grounds), dark themes get the translucent glow look; the in-bubble hours line is white on light, amber on dark.
